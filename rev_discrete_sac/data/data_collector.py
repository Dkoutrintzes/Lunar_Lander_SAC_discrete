...
class Collector(object):
    ...
    def __init__(
        ...
        self.save_entropy_flag = save_entropy
        ...
        # ===== add  entropy  at here =====
        self.action_discrete_flag = isinstance(self.env.action_space,gym.spaces.Discrete) or (isinstance(self.env.action_space,List) and  isinstance(self.env.action_space[0],gym.spaces.Discrete))
    ...
    def _assign_buffer(self, buffer: Optional[ReplayBuffer]) -> None:
        ...
        if buffer is None:
            buffer = VectorReplayBuffer(self.env_num, self.env_num,save_entropy=self.save_entropy_flag)
    ...
    def collect(
        ...
        while True:
            ...
            if random:
                ...
                # ===== add  entropy  at here =====
                if self.save_entropy_flag :
                    if self.action_discrete_flag:
                        old_entropy =[ 1 * np.log(np.prod(self._action_space)) for i in ready_env_ids]
                    else:
                        n = self.env.action_space[0].shape if isinstance(self.env.action_space,List) and  isinstance(self.env.action_space[0],gym.spaces.Box) else self.env.action_space.shape
                        old_entropy = [ -np.prod(n) for i in ready_env_ids]
                    old_entropy = to_numpy(old_entropy)
                    self.data.update(old_entropy=old_entropy)
            else:
                if no_grad:
