...
class DiscreteSACDevPolicy(SACPolicy[TDiscreteSACTrainingStats]):
    """Implementation of SAC for Discrete Action Settings. arXiv:1910.07207.

    :param torch.nn.Module actor: the actor network following the rules in
        :class:`~tianshou.policy.BasePolicy`. (s -> logits)
    :param torch.optim.Optimizer actor_optim: the optimizer for actor network.
    :param torch.nn.Module critic: the first critic network. (s -> Q(s))
    ...
    def _target_q(self, buffer: ReplayBuffer, indices: np.ndarray) -> torch.Tensor:
        ...
        if self.use_avg_q:
            avg_q_target = torch.mean(
                torch.stack([self.critic_old(batch.obs_next), self.critic2_old(batch.obs_next)], dim=-1),
                dim=-1)
        ...
        target_q = target_q.sum(dim=-1) + self.alpha * dist.entropy()
        return target_q
    ...
    def learn(self, batch: Batch, **kwargs: Any) -> Dict[str, float]:
        ...
        # critic 1
        #print(np.shape(batch.obs),np.shape(act))
        ...
        if self.use_clip_q:
            with torch.no_grad():
                q1_old = self.critic_old(batch.obs).gather(1, act).flatten()
        ...
        #print(self.alpha,self.is_auto_alpha,self.clip_q_epsilon,self.use_clip_q,self.use_avg_q,self.use_entropy_penalty,self.entropy_penalty_beta)
        self.sync_weight()
